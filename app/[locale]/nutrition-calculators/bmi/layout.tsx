...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return createCalculatorMetadata(locale, "bmi");
}

export default async function BmiLayout({ children, params }: Props) {
  const { locale } = await params;
  if (!isCalculatorLocale(locale)) notFound();

  return (
    <ScientificCalculatorPage locale={locale} slug="bmi">
      {children}
