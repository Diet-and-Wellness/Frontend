...
const AssessmentProgress = ({
  ...
  return (
    <div className="sticky top-0 z-10 flex flex-col gap-2 bg-surface py-4 sm:py-5">
      <p className="type-label font-medium text-content-muted">
        {t("analysis.progress")}
      </p>

      <div className="flex justify-between items-center">
        <p className="text-2xl font-medium text-brand sm:text-3xl lg:text-[36px]">{progress}%</p>
      ...
      <div className="w-full rounded-full overflow-hidden bg-line-soft">
        <div className="absolute rounded-full w-full h-3 flex justify-evenly items-center">
          {Array.from({ length: total - 1 }).map((_, index) => (
            <div key={index} className="size-1 bg-surface rounded-full" />
          ))}
        </div>
        <motion.div
          animate={{ width: `${progress}%` }}
          transition={{ duration: 0.4, ease: "easeInOut" }}
          className={`${completed ? "bg-brand" : "bg-accent"} p-1.5 rounded-full`}
